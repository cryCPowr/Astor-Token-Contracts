...
import { IChocolatosPool } from "../interfaces/chocolatos/IChocolatosPool.sol";
import { IMasterChocolatos } from "../interfaces/chocolatos/IMasterChocolatos.sol";
import { IVeCho } from "../interfaces/chocolatos/IVeCho.sol";
import { IMCho } from "../interfaces/chocolatos/IMCho.sol";
import { IAsset } from "../interfaces/chocolatos/IAsset.sol";
...
contract ChocolatosStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    struct Pool {
        uint256 pid;                // pid on master Chocolatos
        address depositToken;       // token to be deposited on Chocolatos
        address lpAddress;          // token received after deposit on Chocolatos
    ...
    // Addresses
    address public cho;
    address public veCho;
    address public mCho;

    address public masterChocolatos;
    address public masterAstor;
    ...
    event MasterAstorUpdated(address _oldMasterAstor, address _newMasterAstor);
    event MasterChocolatosUpdated(address _oldChocolatosStaking, address _newChocolatosStaking);
    event SetMCho(address _oldmCho, address _newmCho);
    event SetLockDays(uint256 _oldLockDays, uint256 _newLockDays);

    // Fee
    event AddFee(address _to, uint256 _value, bool _isMCHO, bool _isAddress);
    ...
    // mCho
    event ChoLocked(uint256 _amount, uint256 _lockDays, uint256 _veChoAccumulated);

    // Cho
    event ChoHarvested(uint256 _amount);
    ...
    function __ChocolatosStaking_init(
        address cho,
        address _veCho,
        address _masterChocolatos,
        address _masterAstor
    ) public initializer {
        __Ownable_init();
        cho = _cho;
        veCho = _veCho;
        masterChocolatos = _masterChocolatos;
    ...
    /// @notice get the number of veCho of this contract
    function accumelatedVeCho() external view returns (uint256) {
        return IERC20(veCho).balanceOf(address(this));
    }
    ...
    function expectedVeChoAmount(uint256 amount, uint256 _lockDays) public pure returns (uint256) {
        // veCho = 0.026 * lockDays^0.5
        return amount.wmul(26162237992630200).wmul(LogExpMath.pow(_lockDays * 1e18, 50e16));
    }

    /* ============ External Functions ============ */

    /// @notice deposit Chocolatos pool token in a Chocolatos Pool
    /// @dev this function can only be called by a PoolHelper
    /// @param _lpAddress the lp token to deposit into Chocolatos pool
    ...
    function deposit(
        ...
        IChocolatosPool(poolInfo.depositTarget).deposit(
            depositToken,
            _amount,
            _minimumLiquidity,
            address(this),
        ...
        _toMasterChoAndSendReward(_lpAddress, lpReceived, true); // triggers harvest from Chocolatos exchange
        // update variables
    ...
    function depositLP(
        ...
        // Transfer lp to this contract and stake it to Chocolatos
        IERC20(poolInfo.lpAddress).safeTransferFrom(_for, address(this), _lpAmount);

        _toMasterChoAndSendReward(_lpAddress, _lpAmount, true); // triggers harvest from Chocolatos exchange
    ...
    /// @notice withdraw from a Chocolatos Pool. Note!!! pool helper has to burn receipt token!
    /// @dev Only a PoolHelper can call this function
    /// @param _lpToken the address of the Chocolatos pool lp token
    /// @param _liquidity Chocolatos pool liquidity
    ...
    function withdraw(
        ...
        _toMasterChoAndSendReward(_lpToken, _liquidity, false);

        uint256 beforeWithdraw = IERC20(poolInfo.depositToken).balanceOf(address(this));
        IChocolatosPool(poolInfo.depositTarget).withdraw(
    ...
    /// @notice harvest a Pool from Chocolatos
    /// @param _lpToken Chocolatos pool lp as helper identifier
    function harvest(
        address _lpToken
    ) whenNotPaused _onlyActivePool(_lpToken) external {
        _toMasterChoAndSendReward(_lpToken, 0, true); // triggers harvest from Chocolatos exchange
    }

    /// @notice convert Cho to mCho
    /// @param _amount the number of Cho to convert
    /// @dev the Cho must already be in the contract
    function convertCHO(uint256 _amount) whenNotPaused external returns(uint256) {
        uint256 veChoMintedAmount = 0;
        if (_amount > 0) {
            IERC20(cho).safeApprove(veCho, _amount);
            veChoMintedAmount = IVeCho(veCho).mint(_amount, lockDays);
        }

        emit ChoLocked(_amount, lockDays, veChoMintedAmount);

        return veChoMintedAmount;
    }

    /// @notice stake all the Cho balance of the contract
    function convertAllCho() whenNotPaused external {
        this.convertCHO(IERC20(cho).balanceOf(address(this)));
    }    

    /* ============ Admin Functions ============ */

    /// @notice Register a new Pool on Chocolatos Staking and Master Astor
    /// @dev this function will deploy a new ChocolatosPoolHelper, and add the Pool to the masterAstor
    /// @param _pid the pid of the Pool on master Chocolatos
    /// @param _depositToken the token to stake in the Chocolatos Pool
    /// @param _lpAddress the address of the recepit token after deposit into Chocolatos Pool. Also used for the pool identifier on ChocolatosStaking
    ...
    /// @param _allocPoints the weight of the ASTR allocation
    function registerPool(
        ...
        address rewarder = IMasterAstor(masterAstor).createRewarder(
            address(newToken),
            address(cho)
        );
        IPoolHelper helper = IPoolHelper(
            PoolHelperFactoryLib.createChocolatosPoolHelper(
                _pid,
                address(newToken),
                address(_depositToken),
                address(_lpAddress),
    ...
    /// @notice set the mCho address    
    /// @param _mCho the mCho address
    function setMCho(address _mCho) external onlyOwner {    
        address oldmCho = mCho;
        mCho = _mCho;

        emit SetMCho(oldmCho, mCho);
    }
    ...
    /// @notice update the pool information on Chocolatos deposit and master Astor.
        ...
        _onlyActivePool(_lpAddress)
    {
        ...
        IMasterAstor(masterAstor).set(poolInfo.receiptToken, _allocPoint, _poolHelper, _rewarder, true);

        emit PoolHelperUpdated(_lpAddress);
    }

    function setMasterAstor(address _masterAstor) external onlyOwner {
        address oldMasterAstor = masterAstor;
        masterAstor = _masterAstor;

        emit MasterAstorUpdated(oldMasterAstor, masterAstor);
    }

    function setMasterChocolatos(address _masterChocolatos) external onlyOwner {
        address oldMasterChocolatos = masterChocolatos;
        masterChocolatos = _masterChocolatos;

        emit MasterChocolatosUpdated(oldMasterChocolatos, masterChocolatos);
    }

    function unlockAllVeCho() external whenPaused onlyOwner  {
        IVeCho.Breeding[] memory breedings = IVeCho(veCho).getUserInfo(address(this));
        for (uint256 i = 0; i < breedings.length; i++) {
            if (breedings[i].unlockTime < block.timestamp)
                IVeCho(veCho).burn((i));
        }

        uint256 balance = IERC20(cho).balanceOf(address(this));
        IERC20(cho).safeTransfer(owner(), balance);
    }

    /**
     * @notice pause Chocolatos staking, restricting certain operations
     */
    ...
    /**
     * @notice unpause Chocolatos staking, enabling certain operations
     */
    function unpause() external nonReentrant onlyOwner {
        _unpause();
    }

    /// @notice This function adds a fee to the Astor protocol
    /// @param _value the initial value for that fee
    /// @param _to the address or contract that receives the fee
    /// @param isMCHO true if the fee is sent as MCho, otherwise it will be Cho
    ...
    function addFee(
        ...
        emit AddFee(_to, _value, isMCHO, _isAddress);
    }
    ...
    /// @notice to add bonus token claim from Chocolatos
    ...
    /* ============ Internal Functions ============ */
    function _toMasterChoAndSendReward(address _lpToken, uint256 lpAmount, bool _isStake) internal {
        Pool storage poolInfo = pools[_lpToken];

        address[] memory bonusTokens = assetToBonusRewards[_lpToken];
        uint256 bonusTokensLength = bonusTokens.length;

        uint256 choBeforeBalance = IERC20(cho).balanceOf(address(this));
        uint256[] memory beforeBalances = _rewardBeforeBalances(_lpToken);

        if(_isStake)
            _stakeToChocolatosMaster(_lpToken, lpAmount); // triggers harvest from Chocolatos exchange
        else
            IMasterChocolatos(masterChocolatos).withdraw(poolInfo.pid, lpAmount); // triggers harvest from Chocolatos exchange
        uint256 choRewards = IERC20(cho).balanceOf(address(this)) - choBeforeBalance;
        _sendRewards(cho, poolInfo.rewarder, choRewards);
    ...
    // triggers harvest from Chocolatos exchange
    function _stakeToChocolatosMaster(address _lpToken, uint256 _lpAmount) internal {
        Pool storage poolInfo = pools[_lpToken];
        // Approve Transfer to Master Chocolatos for Staking
        IERC20(_lpToken).safeApprove(masterChocolatos, _lpAmount);
        IMasterChocolatos(masterChocolatos).deposit(poolInfo.pid, _lpAmount);
    }
    ...
    function _sendRewards(
        ...
        for (uint256 i = 0; i < feeInfos.length; i++) {
            ...
            if (feeInfo.isActive) {
                ...
                if (feeInfo.isMCHO && rewardToken == cho) {
                    IERC20(cho).safeApprove(mCho, feeAmount);
                    uint256 beforeBalnce = IMCho(mCho).balanceOf(address(this));
                    IMCho(mCho).convert(feeAmount);
                    rewardToken = mCho;
                    feeTosend = IMCho(mCho).balanceOf(address(this)) - beforeBalnce;
                }
