...
import "./interfaces/IVLASTR.sol";


// Lock ASTR token can exist ONLY in MasterAstor for reward counting
// Master Astor needs to add this as vote Lock ASTR's pool helper as well
// TODO thinking about making this a pool helper as well
/// @title Vote Lock ASTR
/// @author Astor XYZ Team
contract VLASTR is IVLASTR, Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function __vlASTR_init_(
        ...
        __Ownable_init();
        __Pausable_init();
        __ERC20_init("Vote Locked ASTR", "vlASTR");
        if (_maxSlots <= 0)
    ...
    // total ASTR locked, excluding the ones in cool down
    function totalLocked() override public view returns (uint256) {
        return totalSupply() - this.totalAmountInCoolDown();
    }


    /// @notice Get the total ASTR a user locked, not counting the ones in cool down
    /// @param _user the user
    /// @return _lockAmount the total ASTR a user locked, not counting the ones in cool down
    function getUserTotalLocked(address _user) override public view returns (uint256 _lockAmount) {
        // needs fixing
        (uint256 _amountInMasterAstor, ) = IMasterAstor(masterAstor).stakingInfo(address(this), _user);
        _lockAmount = _amountInMasterAstor - getUserAmountInCoolDown(_user);
    }
    ...
    // @notice lock ASTR in the contract
    // @param _amount the amount of ASTR to lock
    ...
    // @param amount the ASTR amount for the slot
    ...
    function startUnlock(uint256 _amountToCoolDown) public override whenNotPaused nonReentrant {
        ...
        IMasterAstor(masterAstor).depositVlASTRFor(0, msg.sender); // just for claiming reward
    ...
    function unlock(uint256 _slotIndex) external override whenNotPaused nonReentrant {
        ...
        IERC20(ASTR).safeTransfer(msg.sender, unlockedAmount);
    ...
    function forceUnLock(uint256 _slotIndex) external whenNotPaused nonReentrant {
        ...
        IERC20(ASTR).safeTransfer(msg.sender, amountToUser);
    ...
    function transferPenalty() external onlyOwner {
        ...
        IERC20(ASTR).safeTransfer(penaltyDestination, totalPenalty);
        totalPenalty = 0;
    ...
    function setMasterChief(address _masterAstor) external onlyOwner {
        if(_masterAstor == address(0)) revert InvalidAddress();
        address oldChief = masterAstor;
        masterAstor = _masterAstor;

        emit NewMasterChiefUpdated(oldChief, masterAstor);
    }
    ...
    function _withdrawAndBurn(uint256 _unlockedAmount) internal {
        IMasterAstor(masterAstor).withdrawVlASTRFor(_unlockedAmount, msg.sender);
    ...
    function _lock(
        ...
        ASTR.safeTransferFrom(spender, address(this), _amount);
        _mint(address(this), _amount);
        _approve(address(this), masterAstor, _amount);
        IMasterAstor(masterAstor).depositVlASTRFor(_amount, _for);
    }
